-- =====================================================
-- FINAL MIGRATION - Handles existing tables
-- =====================================================

-- Step 1: Remove ALL check constraints on user_org_roles
DO $$
DECLARE
...
-- Step 2: Make everyone admin temporarily
UPDATE public.user_org_roles SET role = 'admin';

-- Step 3: Add our constraint
ALTER TABLE public.user_org_roles
  ADD CONSTRAINT user_org_roles_role_check
  CHECK (role IN ('player', 'captain', 'admin'));

-- Step 4: Drop and recreate audit_logs table with correct schema
...
-- Step 5: Create user_invitations table
...
CREATE INDEX idx_user_invitations_club_id ON public.user_invitations(club_id);
...
-- Step 6: Create team_captains table
DROP TABLE IF EXISTS public.team_captains CASCADE;
...
CREATE INDEX idx_team_captains_team_id ON public.team_captains(team_id);
CREATE INDEX idx_team_captains_user_id ON public.team_captains(user_id);
...
-- Step 7: Extend players table (safe - only adds if not exists)
